...
async function updateJobListings(newJobListings) {
    ...
    console.log("updateJobListings function started");


    // Load existing jobs
const existingJobs = await loadExistingJobs();
console.log("Existing Jobs Loaded:", existingJobs.length); // Log the number of jobs loaded
console.log("Sample Existing Job:", existingJobs[0]); // Log a sample job for verification

// Create a map for quick lookup of existing jobs by jobURL
const existingJobsMap = existingJobs.reduce((map, job) => {
    map[job.jobURL] = job;
    return map;
}, {});
console.log("Existing Jobs Map Created:", Object.keys(existingJobsMap).length); // Log the size of the map


const updatedJobs = newJobListings.map((job) => {
    console.log("New Job Listings Received:", newJobListings.length, newJobListings);

    const existingJob = existingJobsMap[job.jobURL];

    if (existingJob) {
        // Retain sponsorship details if applicable
        ...
        if (existingJob.sponsored === 'true') {
            console.log(`Processing sponsorship for job: ${existingJob.jobURL}`);
            // Existing logic for processing sponsorship
            ...
            console.log(`Existing Sponsorship Start Date: ${existingJob.sponsorshipStartDate}`);
            console.log(`Days Sponsored: ${daysSponsored}`);

            if (daysSponsored > 30) {
                job.sponsored = 'false';
                job.sponsorshipStartDate = ''; // Clear sponsorship start date
        ...
        // New jobs: Default sponsorship fields
        job.sponsored = 'false';
        job.category = '';
        job.sponsorshipStartDate = '';
        console.log(`New job added: ${job.jobURL}`);
    }
    ...
    // Retain existing sponsored jobs that are not in the new scrape
    const retainedSponsoredJobs = existingJobs.filter((job) => {
        const isSponsored = job.sponsored === 'true';
        const isNotInNewListings = !newJobListings.some((newJob) => newJob.jobURL === job.jobURL);
        ...
        if (isSponsored && isNotInNewListings) {
            ...
            if (daysSponsored <= 30) {
                console.log(`Retaining sponsored job not in new listings: ${job.jobURL}`);
                return true;
            } else {
                console.log(`Expired sponsorship for job not in new listings: ${job.jobURL}`);
            }
        }
        return false;
    });

    // Combine updated jobs and retained sponsored jobs
    const finalJobList = [...updatedJobs, ...retainedSponsoredJobs];
    console.log("Final Job List to be Written:", finalJobList.length, finalJobList);


    // Write the final job list to the CSV
    console.log("Writing job list to CSV...");
...
async function scrapeAllJobs() {
    console.log("Scraping process started..."); // Add this log here
    let allJobListings = [];
