...
async function updateJobListings(newJobListings) {
    ...
    // Create a map for quick lookups by job URL
    const existingJobsMap = existingJobs.reduce((map, job) => {
        map[job.jobURL] = job;
        return map;
    }, {});

    // Update new jobs or retain sponsorship for existing ones
    const updatedJobs = newJobListings.map((job) => {
        ...
        if (existingJob) {
            ...
            if (existingJob.sponsored === 'true') {
                ...
                if (daysSponsored > 30) {
                    job.sponsored = 'false';
                    job.sponsorshipStartDate = ''; // Clear expired sponsorship start date
                }
            }
        } else {
    ...
    const retainedSponsoredJobs = existingJobs.filter((job) => {
        ...
        if (isSponsored && isNotInNewListings) {
            ...
            return daysSponsored <= 30; // Retain valid sponsorships
        }
        return false;
    });
