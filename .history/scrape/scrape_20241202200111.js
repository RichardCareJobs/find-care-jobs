...
function mergeData(existingJobs, scrapedJobs) {
    ...
    // Retain existing jobs that are not in the current scrape but still valid
    const retainedSponsoredJobs = existingJobs.filter((job) => {
        const isSponsored = job.sponsored === 'true';
        const isNotInScrapedJobs = !scrapedJobsSet.has(`${job.jobURL}-${job.employer}`);
        if (isSponsored && isNotInScrapedJobs) {
    ...
    // Merge existing data with new scraped data
    const mergedData = scrapedJobs.map((scrapedJob) => {
        const existingJob = existingJobs.find(
            (job) => job.jobURL === scrapedJob.jobURL && job.employer === scrapedJob.employer
        );
...
(async () => {
    try {
        ...
        // Step 4: Write final data to CSV
