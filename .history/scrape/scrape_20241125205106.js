...
async function scrapeJobs(url, employer, sector = '') {
    console.log(`Scraping jobs from ${employer} (${url})`);
    const jobListings = [];
    let currentPage = 1;
    ...
    while (hasMoreJobs) {
        try {
            ...
            console.error(`Error scraping ${employer} on page ${currentPage}: ${error.message}`);
            hasMoreJobs = false;
        }
    }

    console.log(`Finished scraping ${employer}. Total jobs: ${jobListings.length}`);
    return jobListings;
}
...
async function scrapeAllJobs() {
    ...
    for (const source of TARGET_URLS) {
        ...
        const scrapedJobs = await scrapeJobs(source.url, source.employer, sector);
