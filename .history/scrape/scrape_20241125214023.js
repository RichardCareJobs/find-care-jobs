...
const TARGET_URLS = [
    ...
    { url: 'https://careers.baptistcare.org.au/jobs/search?query=', employer: 'Baptist Care' }
];
...
async function scrapeBaptistCareJobs(url, employer) {
    ...
    while (hasMoreJobs) {
        ...
        jobsOnPage.each((index, element) => {
            ...
            const jobURL = $(element).find('.card-title.job-search-results-card-title a').attr('href');
            const location = $(element).find('job-component-list.job-component-list-location').text().trim();
            const jobCategory = $(element).find('.job-component-icon-and-text.job-component-dropdown-field-1').text().trim();
            const jobType = $(element).find('.job-component-list.job-component-list-employment_type').text().trim();
            ...
            const jobData = {
                ...
                sector: 'Aged Care',
                jobType,
                jobURL: jobURL ? 'https://careers.baptistcare.org.au/jobs/search?query=' + jobURL : 'Not specified',
                employer,
                sponsored: 'false',
                category: '',
                scrapeDate
