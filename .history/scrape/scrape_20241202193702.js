...
function mergeData(existingJobs, scrapedJobs) {
    ...
    // Retain existing sponsored jobs not present in the current scrape
    const retainedSponsoredJobs = existingJobs.filter(job => {
        const isSponsored = job.sponsored === 'true';
        const isNotInScrapedJobs = !scrapedJobs.some(scrapedJob =>
            scrapedJob.jobURL === job.jobURL && scrapedJob.employer === job.employer
        );
