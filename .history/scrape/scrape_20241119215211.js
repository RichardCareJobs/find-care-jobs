...
async function updateJobListings(newJobListings) {
    ...
    const updatedJobs = newJobListings.map((job) => {
        // Find existing job based on unique identifier (e.g., jobURL and employer)
        const existingJob = existingJobs.find(
            (ej) => ej.jobURL === job.jobURL && ej.employer === job.employer
        );

        if (existingJob) {
            // Retain sponsorship status, category, and sponsorship start date from the existing job
            ...
            // Check if the sponsorship has expired
            if (existingJob.sponsored === 'true') {
                ...
                if (daysSponsored > 30) {
                    // Expire sponsorship if over 30 days
                    job.sponsored = 'false';
                    job.sponsorshipStartDate = ''; // Clear start date for expired sponsorships
                }
            }
        } else {
            // For new jobs, default sponsorship status and category
            job.sponsored = 'false';
            job.category = '';
    ...
    // Retain existing jobs that are sponsored and not in the new listings
    const retainedSponsoredJobs = existingJobs.filter((job) => {
        ...
        if (isSponsored && isNotInNewListings) {
            ...
            // Retain only if sponsorship is within 30 days
            return daysSponsored <= 30;
        }
        return false;
    });

    // Combine updated jobs with retained sponsored jobs
    const finalJobList = [...updatedJobs, ...retainedSponsoredJobs];

    // Write the final job listings to CSV
...
async function scrapeOpalHealthcareJobs(url, employer) {
    ...
    while (hasMoreJobs) {
        ...
        jobsOnPage.each((index, element) => {
            ...
            const jobURL = jobPath ? 'https://globalaus242.dayforcehcm.com' + jobPath : 'Not specified';
