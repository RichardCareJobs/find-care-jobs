...
function mergeData(existingJobs, scrapedJobs) {
    ...
    const mergedData = scrapedJobs.map((scrapedJob) => {
        ...
        const existingJob = existingJobsMap[key];
    ...
    const retainedSponsoredJobs = existingJobs.filter((job) => {
        const key = `${job.jobURL}-${job.employer}`;
        const isNotInScrapedJobs = !scrapedJobs.some((scrapedJob) => key === `${scrapedJob.jobURL}-${scrapedJob.employer}`);
        const isSponsored = job.sponsored === 'true';

        if (isNotInScrapedJobs && isSponsored) {
            const sponsorshipStartDate = new Date(job.sponsorshipStartDate);
            const daysSponsored = (new Date() - sponsorshipStartDate) / (1000 * 60 * 60 * 24);
            return daysSponsored <= 30;
        }

        return false;
    });

    return [...mergedData, ...retainedSponsoredJobs];
}
...
function arrangeJobsForPage(sponsoredJobs, regularJobs, pageSize, pageIndex) {
    const startIndex = pageIndex * pageSize;
    const endIndex = startIndex + pageSize;

    let currentRegularJobs = regularJobs.slice(startIndex, endIndex);
    let arrangedJobs = [];

    for (let i = 0; i < pageSize; i++) {
        if (i === 0 || i === 4) { // Positions 1 and 5 (0-indexed)
            if (sponsoredJobs.length > 0) {
                arrangedJobs.push(sponsoredJobs.shift());
            } else {
                arrangedJobs.push(currentRegularJobs.shift());
            }
        } else {
            arrangedJobs.push(currentRegularJobs.shift());
        }
...
function groupAndRandomizeJobsByDateWithPlacement(jobs, pageSize, pageIndex) {
    const jobsByDate = jobs.reduce((acc, job) => {
        const date = job.scrapeDate;
        if (!acc[date]) acc[date] = [];
        acc[date].push(job);
    ...
    // Separate into sponsored and regular jobs
    const sponsoredJobs = randomizedJobs.filter(job => job.sponsored === 'true');
    const regularJobs = randomizedJobs.filter(job => job.sponsored !== 'true');

    return arrangeJobsForPage(sponsoredJobs, regularJobs, pageSize, pageIndex);
}
...
(async () => {
    try {
        ...
        const mergedJobs = mergeData(existingJobs, allJobListings);

        // Step 4: Randomize jobs by scrape date
        const randomizedJobs = groupAndRandomizeJobsByDate(mergedJobs);

        // Step 5: Apply Sponsored Placement Logic for All Pages
        const pageSize = 10;
        const totalPages = Math.ceil(randomizedJobs.length / pageSize);
        let finalJobList = [];
        for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
            const sponsoredJobs = randomizedJobs.filter(job => job.sponsored === 'true');
            const regularJobs = randomizedJobs.filter(job => job.sponsored !== 'true');
            const arrangedPage = arrangeJobsForPage(sponsoredJobs, regularJobs, pageSize, pageIndex);
            finalJobList = finalJobList.concat(arrangedPage);
        }

        // Step 6: Write the final job list to the CSV
