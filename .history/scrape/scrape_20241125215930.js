...
async function scrapeBaptistCareJobs(url, employer) {
    ...
    while (hasMoreJobs) {
        try {
            console.log(`Scraping Baptist Care on page ${currentPage}...`); // Add logging
            const response = await axios.get(`${url}?page=${currentPage}`);
            const html = response.data;
            const $ = cheerio.load(html);

            const jobsOnPage = $('.job-search-results-card'); // Update this selector
            ...
            jobsOnPage.each((index, element) => {
                const jobTitle = $(element).find('.card-title a').text().trim(); // Confirm the exact selector
                const jobPath = $(element).find('.card-title a').attr('href');
                ...
                const jobCategory = $(element).find('.job-component-icon-and-text').text().trim(); // Update if needed
                ...
                const jobData = {
                    ...
                    closingDate: 'Not specified', // Add if available
                    jobURL,
                    employer,
                    sponsored: 'false',
                ...
                console.log(`Found job: ${jobTitle} at ${location}`);
                jobListings.push(jobData);
            });
