...
async function safeScrape(fn, url, employer) {
  try {
    return await fn(url, employer);
  } catch (err) {
    console.error(`⚠️  Failed to scrape ${employer}:`, err.message);
    return [];
  }
}
...
(async () => {
    try {
        ...
        let scrapedJobs = [];
        scrapedJobs = scrapedJobs.concat(await safeScrape(scrapeUnitingCareAgedCare, TARGET_URLS[0].url, TARGET_URLS[0].employer));
        scrapedJobs = scrapedJobs.concat(await safeScrape(scrapeUnitingCareDisabilitySupport, TARGET_URLS[1].url, TARGET_URLS[1].employer));
        scrapedJobs = scrapedJobs.concat(await safeScrape(scrapeHammondCareJobs, TARGET_URLS[2].url, TARGET_URLS[2].employer));
        scrapedJobs = scrapedJobs.concat(await safeScrape(scrapeWhiddonJobs, TARGET_URLS[3].url, TARGET_URLS[3].employer));
        scrapedJobs = scrapedJobs.concat(await safeScrape(scrapeLifeWithoutBarriersJobs, TARGET_URLS[4].url, TARGET_URLS[4].employer));
        scrapedJobs = scrapedJobs.concat(await safeScrape(scrapeLifestyleSolutionsJobs, TARGET_URLS[5].url, TARGET_URLS[5].employer));
        scrapedJobs = scrapedJobs.concat(await safeScrape(scrapeOpalHealthcareJobs, TARGET_URLS[6].url, TARGET_URLS[6].employer));
        scrapedJobs = scrapedJobs.concat(await safeScrape(scrapeBaptistCareJobs, TARGET_URLS[7].url, TARGET_URLS[7].employer));
        ...
        // count the number of existing jobs
