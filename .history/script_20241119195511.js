...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performSearch() {
        ...
        const sector = document.getElementById('sector').value; // Get selected sector
        ...
        Papa.parse('scrape/job_listings.csv', {
            ...
            complete: function(results) {
                const filteredJobs = results.data.filter(job => {
                    ...
                    const jobSector = job['Sector'] || ''; // Assume sector data is in the 'Sector' column
                    return (
                        (job['Job Title'].toLowerCase().includes(jobTitle) || jobTitle === '') &&
                        (jobLocation.includes(location) || location === '') &&
                        (jobSector === sector || sector === '') // Filter by sector
                    );
                });
