...
let preRandomizedJobs = []; // Store pre-randomized jobs for pagination
...
function fetchJobListings() {
    Papa.parse('scrape/job_listings.csv', {
        ...
        complete: function (results) {
            const jobs = results.data;
            preRandomizeJobs(jobs); // Pre-randomize jobs
...
// Pre-randomize jobs for efficient infinite scroll
function preRandomizeJobs(jobs) {
    ...
    Object.keys(jobsByDate)
        ...
        .forEach(date => {
            ...
            preRandomizedJobs.push(...shuffledJobs);
        });
}

// Arrange jobs to enforce sponsored placement (1st and 5th positions)
...
function displayJobListings(jobs) {
    ...
    // Arrange sponsored jobs dynamically
